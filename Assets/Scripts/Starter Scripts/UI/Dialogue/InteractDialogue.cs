...
public class InteractDialogue : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!hasBeenUsed && inArea && Input.GetMouseButtonDown(0) && nextTime < Time.timeSinceLevelLoad)
        {
            ...
            {
                manager.AdvanceDialogue(endGame);
            }
        }
    }

    /* Called when you want to start dialogue */
    void TriggerDialogue(bool endGame)
    {
        ReadTextFile(); // loads in the text file
        manager.StartDialogue(dialogue, endGame); // Accesses Dialogue Manager and Starts Dialogue
    }
