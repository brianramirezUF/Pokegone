...
public class BattleStartManager : MonoBehaviour
{
    ...
    */
    ...
    [HideInInspector]
    public BattleDialogue currentTrigger;
    ...
    public void StartDialogue(Queue<string> dialogue, GameObject BattleCamera, BattleDialogBox battle)
    {
        isInDialouge = true;
        ...
        PrintDialogue(BattleCamera, battle); // Prints out the first line of dialogue
    }

    public void AdvanceDialogue(GameObject BattleCamera, BattleDialogBox battle) // call when a player presses a button in Dialogue Trigger
    {
        PrintDialogue(BattleCamera, battle);
    }

    private void PrintDialogue(GameObject BattleCamera, BattleDialogBox battle)
    {
        if (inputStream.Peek().Contains("EndQueue")) // special phrase to stop dialogue
        {
            ...
            {
                inputStream.Dequeue();
                EndDialogue(BattleCamera, battle);
            }
            else
            {
                cancelTyping = true;
        ...
        else if (inputStream.Peek().Contains("[NAME=")) //Set the name of the speaker
        {
            ...
            NameText.text = name;
            PrintDialogue(BattleCamera, battle); // print the rest of this line
        }
    ...
    public void EndDialogue(GameObject BattleCamera, BattleDialogBox battle)
    {
        TextBox.text = "";
        NameText.text = "";
        inputStream.Clear();
        ...
        cancelTyping = false;
        isTyping = false;
        //currentTrigger.battleDialogue.SetActive(false);
        // isOpen = false;
        if (freezePlayerOnDialogue)
        {
            //UnFreezePlayer();
        ...
        if(currentTrigger.inBattle == false)
        {
            currentTrigger.inBattle = true;
    ...
    public void EndDialogueWalkAway()
    {
        TextBox.text = "";
        NameText.text = "";
        inputStream.Clear();
